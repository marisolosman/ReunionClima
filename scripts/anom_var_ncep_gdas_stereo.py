...
def plot_anomaly(
    ...
    start_date: dt.date,
    end_date: dt.date,
    latmin: float,   # Se usará como el límite del mapa (latr)
    levcont: float,
    levint: float,
) -> str:
    if levint <= 0 or levcont <= 0:
        raise ValueError("Los intervalos y niveles de contorno deben ser mayores a 0")

    # 1. Configuración de la figura y proyección
    fig = plt.figure(figsize=(16, 11))
    # Proyección estereográfica polar sur centrada en 300° (60°W)
    projection = ccrs.SouthPolarStereo(central_longitude=300)
    ax = plt.subplot(projection=projection)

    # 2. Manejo del punto cíclico para evitar el hueco en 0/360 long
    # Squeeze para asegurar que sea 2D (lat, lon)
    data_2d = np.squeeze(anomaly)
    data_cyclic, lon_cyclic = add_cyclic_point(data_2d, coord=lon)
    lons, lats = np.meshgrid(lon_cyclic, lat)

    # 3. Definición de niveles de contorno
    clevs = np.arange(-levcont, levcont + levint, levint)

    # 4. Establecer extensión y límite circular
    # Usamos latmin como el "latr" (límite del círculo)
    crs_latlon = ccrs.PlateCarree()
    ax.set_extent([0, 359.9, -90, latmin], crs=crs_latlon)

    # Crear el borde circular
    ...
    # 6. Estética
    plt.colorbar(im, fraction=0.052, pad=0.04, shrink=0.8, aspect=12).set_label(units)
    
    ax.add_feature(cfeature.COASTLINE)
    ax.add_feature(cfeature.BORDERS, linestyle='-', alpha=.5)
    ax.gridlines(crs=crs_latlon, linewidth=0.3, linestyle='-')

    titulo = (f"Anomalías {plot_name} ({dataset_id}) {level_text}\n"
              f"{start_date:%d/%m/%Y} - {end_date:%d/%m/%Y}")
    ax.set_title(titulo)

    # 7. Guardado de archivo
    output_name = build_output_name(
        dataset_id, level_text, start_date, end_date, 
        latmin
    )
